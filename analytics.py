...
class AnalyticsTracker:
    ...
    def __init__(self, cache_manager: CacheManager, analytics_file: str = "cache/analytics.json"):
        ...
        self.rerolls_per_bucket: Dict[str, int] = {"old": 0, "medium": 0, "new": 0, "all": 0}
        ...
        if self.analytics_file.exists():
            try:
                with open(self.analytics_file, 'r') as f:
                    ...
                    self.rerolls_per_bucket = data.get('rerolls_per_bucket', {"old": 0, "medium": 0, "new": 0, "all": 0})
                    self.times_shown_per_album = {int(k): v for k, v in data.get('times_shown_per_album', {}).items()}
            except Exception:
    ...
    def increment_album_shown(self, collection_id: int):
        ...
        with self._lock:
            self.times_shown_per_album[collection_id] = self.times_shown_per_album.get(collection_id, 0) + 1
    ...
    def increment_and_save_sometimes(self, bucket: str, collection_id: int, save_every: int = 50):
